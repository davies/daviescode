...
unsigned long get_audio(FILE *musicin,short FAR buffer[2][1152],
...
void window_subband(sample_t FAR **buffer,double FAR z[HAN_SIZE],int k)
{
...
void create_ana_filter(double FAR filter[SBLIMIT][64])
{
	register int i,k;
...
void filter_subband(double FAR z[HAN_SIZE], double s[SBLIMIT])
{
	double y[64];
	int i,j;
	static char init = 0;
...
void II_combine_LR(double FAR sb_sample[2][3][SCALE_BLOCK][SBLIMIT],
	double FAR joint_sample[3][SCALE_BLOCK][SBLIMIT],int sblimit)
...
void II_scale_factor_calc(double FAR sb_sample[][3][SCALE_BLOCK][SBLIMIT],
						  unsigned int scalar[][3][SBLIMIT],int stereo,int sblimit)
{
	int i,j, k,t;
	double s[SBLIMIT];
...
void pick_scale(unsigned int scalar[2][3][SBLIMIT],frame_params *fr_ps,double FAR max_sc[2][SBLIMIT])
{
	int i,j,k,max;
...
void put_scale(unsigned int scalar[2][3][SBLIMIT],frame_params *fr_ps,double FAR max_sc[2][SBLIMIT])
{
	int i,k;
	int stereo  = fr_ps->stereo;
	int sblimit = fr_ps->sblimit;
...
void II_transmission_pattern(unsigned int scalar[2][3][SBLIMIT],unsigned int scfsi[2][SBLIMIT],frame_params *fr_ps)
{
	int stereo  = fr_ps->stereo;
	int sblimit = fr_ps->sblimit;
	int dscf[2];
	...
	for (k=0;k<stereo;k++)
		for (i=0;i<sblimit;i++) {
			...
			for (j=0;j<2;j++) {
				if (dscf[j]<=-3) classes[j] = 0;
				else if (dscf[j] > -3 && dscf[j] <0) classes[j] = 1;
				else if (dscf[j] == 0) classes[j] = 2;
				else if (dscf[j] > 0 && dscf[j] < 3) classes[j] = 3;
				else classes[j] = 4;
			}
			switch (pattern[classes[0]][classes[1]]) {
...
void II_encode_scale(unsigned int bit_alloc[2][SBLIMIT], unsigned int scfsi[2][SBLIMIT],
					 unsigned int scalar[2][3][SBLIMIT],frame_params *fr_ps,Bit_stream_struc *bs)
{
	int stereo  = fr_ps->stereo;
	int sblimit = fr_ps->sblimit;
...
int II_bits_for_nonoise(double FAR perm_smr[2][SBLIMIT],unsigned int scfsi[2][SBLIMIT],frame_params *fr_ps)
{
	int sb,ch,ba;
	int stereo  = fr_ps->stereo;
	int sblimit = fr_ps->sblimit;
...
void II_main_bit_allocation(double FAR perm_smr[2][SBLIMIT],unsigned int scfsi[2][SBLIMIT],
							unsigned int bit_alloc[2][SBLIMIT],int *adb,frame_params *fr_ps)
{
	int  noisy_sbs;
...
int II_a_bit_allocation(double FAR perm_smr[2][SBLIMIT],unsigned int scfsi[2][SBLIMIT],unsigned int bit_alloc[2][SBLIMIT],
...
void II_subband_quantization(unsigned int scalar[2][3][SBLIMIT],double FAR sb_samples[2][3][SCALE_BLOCK][SBLIMIT],
							 unsigned int j_scale[3][SBLIMIT],double FAR j_samps[3][SCALE_BLOCK][SBLIMIT],
							 unsigned int bit_alloc[2][SBLIMIT],unsigned int FAR sbband[2][3][SCALE_BLOCK][SBLIMIT],
							 frame_params *fr_ps)
{
			...
			for (i=0;i<sblimit;i++){
				for (k=0;k<((i<jsbound)?stereo:1);k++){
					if (bit_alloc[k][i]) {
						...
						n = 0;
						while  ( ( (unsigned long)(1L<<(long)n) < ((unsigned long) ((*alloc)[i][bit_alloc[k][i]].steps)& 0xffff)) && ( n <16) )
							n++;
						n--;
...
void II_encode_bit_alloc(unsigned int bit_alloc[2][SBLIMIT],frame_params *fr_ps,Bit_stream_struc *bs)
{
	int i,k;
	int stereo  = fr_ps->stereo;
	int sblimit = fr_ps->sblimit;
...
void II_sample_encoding(unsigned int FAR sbband[2][3][SCALE_BLOCK][SBLIMIT],unsigned int bit_alloc[2][SBLIMIT],
