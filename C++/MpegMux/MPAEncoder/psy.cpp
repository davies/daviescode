#include "common.h"
#include "encoder.h"
#include "absthr_tables.h"

void psycho_anal(sample_t *buffer,short int savebuf[1056],int chn, int lay,FLOAT snr32[32],double sfreq)        /* to match prototype : float args are always double */
{
	unsigned int   i, j, k;
	...
	/* The static variables "r", "phi_sav", "newV", "old" and "oldest" have    */
	/* to be remembered for the unpredictability measure.  For "r" and        */
	/* "phi_sav", the first index from the left is the channel select and     */
	/* the second index is the "age" of the data.                             */
	
	static int     newV = 0, old = 1, oldest = 0;
	...
	static FLOAT *absthr_tables[3] = {absthr_table_0,absthr_table_1,absthr_table_2};
	...
	if(init==0){
		...
		absthr = absthr_tables[sfreq_idx];
	...
	case 2:
		...
		* up with center of 576 "newV" audio samples.                                *
		*                                                                           *
		* For layer 1, the input data still needs to be staggered by 256 samples,   *
		* then it must be staggered again so that the 384 "newV" samples are centered*
		* in the 1024 FFT window.  The net offset is then 576 and you need 448 "newV"*
		* samples for each iteration to keep the 384 samples of interest centered   *
			...
			*****************************************************************************/
			...
			if(lay==2 || (lay==1 && chn==0) ){
				if(newV==0){newV = 1; oldest = 1;}
				else {newV = 0; oldest = 0;}
				if(old==0)old = 1; else old = 0;
			}
			for(j=0; j<HBLKSIZE; j++){
				r_prime = 2.0 * r[chn][old][j] - r[chn][oldest][j];
				phi_prime = 2.0 * phi_sav[chn][old][j] - phi_sav[chn][oldest][j];
				r[chn][newV][j] = sqrt((double) energy[j]);
				phi_sav[chn][newV][j] = phi[j];
				temp1=r[chn][newV][j] * cos((double) phi[j]) - r_prime * cos((double) phi_prime);
				temp2=r[chn][newV][j] * sin((double) phi[j]) - r_prime * sin((double) phi_prime);
				temp3=r[chn][newV][j] + fabs((double)r_prime);
